...
const TEXTS = ['UX Designer', 'Visual Designer', 'Frontend Developer'];
    ...
    <>
      <div className=" relative">
        <section className=" relative">
          <div className=" relative z-30 bg-shade_3 bg-opacity-40 border-[0.002px] border-shade_3   flex flex-col gap-5 p-5 rounded-xl">
            <div className="flex flex-wrap gap-5 justify-center w-full">
              <div className="bg-gradient-to-r from-shade_1/80  to-shade_1/80  grow shrink basis-[120px] max-w-24 md:max-w-28 flex aspect-square justify-center rounded-md overflow-hidden">
              ...
              <div className=" grow shrink basis-[300px] w-full flex-col flex gap-2">
                ...
                <div className=" flex flex-col gap-2">
                  <p className="text-[#e6e6e6] text-2xl font-semibold text-center sm:text-left">
                    Siddhant Bilange
                  </p>
                  <p className="text-shade_5 text-center mx-auto md:mx-0">
                    <div className=" flex">
                    I'm a &nbsp;<TextTransition springConfig={presets.wobbly} className=" text-shade_1 font-semibold">{TEXTS[index % TEXTS.length]}</TextTransition>
                    </div>
                  </p>
                </div>
              </div>
            </div>
            <div className=" flex-wrap gap-2 flex rounded-md p-2 text-sm md:text-base">
              {tags.map((item, index) => (
                <span
                  key={index}
                  className="text-shade_5 text-nowrap flex items-center gap-1 bg-shade_3 bg-opacity-50 px-3 py-2 rounded-2xl "
                >
                  <span className="text-shade_1">{item.icon}</span>
                  {item.text}
                </span>
              ))}
            </div>
            <div className="flex w-full flex-wrap gap-2  text-sm md:text-base">
              <NavLink
                to="https://www.linkedin.com/in/siddhant-bilange"
                target="_blank"
                className="w-full grow shrink basis-[200px] "
              >
                <div className="w-full bg-shade_5  hover:bg-opacity-90 transition ease-out duration-100 p-3 rounded-lg flex justify-center items-center gap-3 ">
                  ...
                  <span className="text-shade_2 font-semibold">Siddhant Bilange</span>
                </div>
              </NavLink>

              <NavLink
                to="mailto:siddhantbilange@gmail.com"
                target="_blank"
                className="w-full grow shrink basis-[200px] "
              >
                <div className="w-full bg-shade_5  hover:bg-opacity-90 transition ease-out duration-100 p-3 rounded-lg flex justify-center items-center gap-3 ">
                  <svg width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M18 0H2C0.9 0 0.00999999 0.9 0.00999999 2L0 14C0 15.1 0.9 16 2 16H18C19.1 16 20 15.1 20 14V2C20 0.9 19.1 0 18 0ZM18 4L10 9L2 4V2L10 7L18 2V4Z" fill="#050505" />
                  </svg>
                  <span className="text-shade_2 font-semibold">siddhantbilange@gmail.com</span>
                </div>
              </NavLink>
            </div>
        ...
        {/* <FlickeringGrid
              className="z-0 absolute top-0 left-0   [mask-image:radial-gradient(450px_circle_at_center,white,transparent)]"
              squareSize={4}
              gridGap={6}
              color="#60A5FA"
              maxOpacity={0.3}
